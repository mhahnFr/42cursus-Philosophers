...
enum e_state	philo_sleep_or_die(struct s_philo *this, int time)
{
	...
	gettimeofday(&t, NULL);
	if (time > this->delegate->time_to_die) // FIXME cannot be that inaccurate!!!
	{
...
enum e_state	philo_eat(struct s_philo *this, int time)
{
	...
	usleep(time * 1000);
	gettimeofday(&t, NULL);
//	if (delegate_get_time_diff(&t, &this->last_eat_time) >= this->delegate->time_to_die) // FIXME init las eat time!!!
//		return (DIED);
